...
import { useUpdateFavourite } from "./use-update-favourite";
...
function BoardsListPage() {
  ...
  const { toggle } = useUpdateFavourite();

  const handleToggleFavorite = (board: ApiSchemas["Board"]) => {
    toggle({id: board.id, isFavorite: board.isFavorite});
  };
      ...
      <div className="mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="md:col-span-3">
          ...
          <Input
            ...
            value={boardsListFilters.search}
            onChange={(e) => boardsListFilters.setSearch(e.target.value)}
            className="w-full"
          />
        </div>
              ...
              <Card key={board.id} className="relative">
                ...
                <CardFooter>
                  <Button
                    ...
                    onClick={() =>
                      deleteBoardMutation.deleteBoardMutation(board.id)
                    }
                  >
                    Удалить
                  </Button>
